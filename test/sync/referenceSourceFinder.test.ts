...
function makeExtractor(track: SubtitleTrack | null, onExtract?: (out: string) => void): MkvExtractorInterface {
    return {
        ...
        findSubtitleTrack: jest.fn().mockResolvedValue({ track, bitmapOnlyLanguages: [], unsupportedLanguages: [] }),
...
describe("ReferenceSourceFinder — embedded tracks", () => {
    ...
    it("explains that image-based tracks cannot be used, instead of reporting nothing found", async () => {
        ...
        const extractor: MkvExtractorInterface = {
            listSubtitleTracks: jest.fn().mockResolvedValue([]),
            findSubtitleTrack: jest.fn().mockResolvedValue({ track: null, bitmapOnlyLanguages: ["fr", "en"], unsupportedLanguages: [] }),
            hasSubtitleTrack: jest.fn().mockResolvedValue(false),
            extractSubtitle: jest.fn()
        };
    ...
    it("explains that an ASS/SSA track cannot be read, and does not call it image-based", async () => {
        // mkvextract writes ASS in its native format whatever the output filename, so
        // extracting one to a .srt path yields a file the parser reads as zero entries.
        const target = touch("Movie.Hebrew.srt");
        touch("Movie.mkv");
        const extractor: MkvExtractorInterface = {
            listSubtitleTracks: jest.fn().mockResolvedValue([]),
            findSubtitleTrack: jest.fn().mockResolvedValue({ track: null, bitmapOnlyLanguages: [], unsupportedLanguages: ["en"] }),
            hasSubtitleTrack: jest.fn().mockResolvedValue(false),
            extractSubtitle: jest.fn()
        };
        ...
        expect(lookup.source).toBeNull();
        expect(lookup.reason).toContain("ASS/SSA");
        expect(lookup.reason).not.toContain("image-based");
        expect(extractor.extractSubtitle).not.toHaveBeenCalled();
    });
    ...
    it("prefers a usable sidecar over reporting bitmap-only tracks", async () => {
        ...
        const extractor: MkvExtractorInterface = {
            listSubtitleTracks: jest.fn().mockResolvedValue([]),
            findSubtitleTrack: jest.fn().mockResolvedValue({ track: null, bitmapOnlyLanguages: ["fr"], unsupportedLanguages: [] }),
            hasSubtitleTrack: jest.fn().mockResolvedValue(false),
            extractSubtitle: jest.fn()
        };
