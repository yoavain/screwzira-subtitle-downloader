...
const PERSISTENT_CACHE_DIR: string = path.resolve(__dirname, "..", "..", "resources", "cache", "persistent");
const TRANSIENT_CACHE_DIR: string = path.resolve(__dirname, "..", "..", "resources", "cache", "transient");

describe("test tvShowId cache", () => {
    it("Test - not found", async () => {
        const mockFetch = jest.fn(async () => undefined);
        const tvShowIdCache = new TvShowIdCache(uuid(), TRANSIENT_CACHE_DIR);
    ...
    it("Test persistence", async () => {
        ...
        const mockFetch = jest.fn(async () => "tvShowId1");
        const tvShowIdCache = new TvShowIdCache(id, PERSISTENT_CACHE_DIR);
