...
export function timeWarp(target: TimeSpan[], reference: TimeSpan[], options: TimeWarpOptions = {}): TimeWarp {
    ...
    // Scoring runs against a merged copy of the reference, not the raw spans.
    //
    // spanOverlap sums per reference span, so two reference spans that overlap each other made
    // their shared milliseconds count twice — which contradicts "milliseconds covered by any
    // reference span" and inflated confidence. In the other direction, firstCandidate steps
    // back exactly one span, so a still-open earlier span was missed. Real subtitles overlap
    // (the Office fixture has eight such pairs, one of them two identical spans), so neither
    // was hypothetical. Merging once fixes the double count, the undercount, and the unstated
    // assumption that the reference arrives sorted.
    //
    // refineOffset deliberately keeps using the ORIGINAL spans: it snaps to a real entry
    // boundary, and a merged span's start is not always one.
    const merged = mergeSpans(reference);
    const refStarts = Float64Array.from(merged, (s) => s.start);
    const refEnds = Float64Array.from(merged, (s) => s.end);
    const entryStarts = Float64Array.from([...reference].sort((a, b) => a.start - b.start), (s) => s.start);
    ...
    for (const ratio of new Set(opts.framerateRatios)) {
        ...
        const offsets = candidateOffsets(scaled, entryStarts, opts);
        if (offsets.length === 0) {
            continue;
        }
        ...
        const segments: Segment[] = runs.map((run) => ({
            ...
            endIdx: run.endIdx,
            scale: ratio,
            offset: refineOffset(scaled, run.startIdx, run.endIdx, offsets[run.offsetIdx], entryStarts, refStarts, refEnds, opts.offsetBinMs)
        }));
...
/** Index of the value in `sorted` closest to `value`, or -1 when it is empty. */
function nearestStart(value: number, sorted: Float64Array): number {
    if (sorted.length === 0) {
        return -1;
    }
    const j = lowerBound(sorted, value);
    if (j === 0) {
        return 0;
    }
    if (j >= sorted.length) {
    ...
    return value - sorted[j - 1] <= sorted[j] - value ? j - 1 : j;
}

/** Sorted, non-overlapping copy of `spans`. Touching spans are fused; zero-length ones dropped. */
...
function refineOffset(
    ...
    for (let i = startIdx; i <= endIdx; i++) {
        // Require a real overlap against the merged spans, then snap to the nearest ORIGINAL
        // entry start. A merged span's boundary is not always a boundary any entry has.
        if (bestOverlappingRef(target[i].start + offset, target[i].end + offset, refStarts, refEnds) < 0) {
            continue;
        }
        const j = nearestStart(target[i].start + offset, entryStarts);
        if (j >= 0) {
            diffs.push(entryStarts[j] - target[i].start);
        }
    }
    if (diffs.length === 0) {
        return offset;
