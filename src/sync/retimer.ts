...
export function repair(entries: SubtitleEntry[], options: RetimeOptions = {}, original?: SubtitleEntry[]): SubtitleEntry[] {
    ...
    for (let i = 0; i < out.length; i++) {
        // A negative shift is a normal outcome, not an edge case — the fit routinely returns
        // one. Without this floor an entry near the start of the file lands before zero, and
        // SRT has no way to express that: the writer emits "-1:-1:-1,-500" and players reject
        // the file. Clamp before the duration rule so the entry keeps its length.
        if (out[i].start < 0) {
            out[i].start = 0;
        }
