...
describe("repair - times never go below zero", () => {
    // A negative shift is a normal fit outcome, so an entry near the start of the file can
    // land before zero. SRT has no way to express that.
    it("clamps a negative start to zero and keeps a valid duration", () => {
        const out = repair([entry(1, -3000, -1000)]);

        expect(out[0].start).toBe(0);
        expect(out[0].end).toBeGreaterThan(0);
    });

    it("clamps every entry pushed before zero, keeping order", () => {
        const out = repair([entry(1, -5000, -3000), entry(2, -2000, -500), entry(3, 1000, 3000)]);

        for (const e of out) {
            expect(e.start).toBeGreaterThanOrEqual(0);
            expect(e.end).toBeGreaterThan(e.start);
        }
        expect(out[0].start).toBeLessThanOrEqual(out[1].start);
        expect(out[1].start).toBeLessThanOrEqual(out[2].start);
    });

    it("leaves a non-negative list untouched", () => {
        const entries = [entry(1, 1000, 3000), entry(2, 4000, 6000)];
        expect(repair(entries)).toEqual(entries);
    });
});
